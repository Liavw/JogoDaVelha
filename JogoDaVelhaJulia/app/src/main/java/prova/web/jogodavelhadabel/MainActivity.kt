...
class MainActivity : AppCompatActivity() {
    ...
    fun buttonClick(view: View) {
        ...
        val drawableResource = if (jogadorAtual == "cavera") R.drawable.cavera else R.drawable.onepiece
        ...
        if (!vencedor.isNullOrBlank()) {
            ...
            // Alterna jogador
            jogadorAtual = if (jogadorAtual == "cavera") "onepiece" else "cavera"
            ...
            if (contraMaquina && jogadorAtual == "onepiece") {
        ...
        for (i in 0..2) {
            for (j in 0..2) {
                if (tabuleiro[i][j].isEmpty()) {
                    ...
                    tabuleiro[i][j] = if (jogadorAtual == "cavera") "onepiece" else "cavera"
                    if (verificaVencedor(tabuleiro) == if (jogadorAtual == "cavera") "onepiece" else "cavera") {
                        tabuleiro[i][j] = ""
                        return Pair(i, j)
                    }
                    tabuleiro[i][j] = ""
    ...
    fun verificaVencedor(tabuleiro: Array<Array<String>>): String? {
        ...
        for (linha in tabuleiro) {
            for (valor in linha) {
                if (valor == "cavera" || valor == "onepiece") {
                    empate++
                }
            }
        }
